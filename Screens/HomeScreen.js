...
const HomeScreen = ({ navigation }) => {
  ...
  const generateRandomTask = async () => {
    try {
        ...
        const response = await fetch(`http://tasks1-env.eba-tihfzwsy.us-east-1.elasticbeanstalk.com/api/Tasks/CreateRandomTask?idn=${idn}`, {
        method: 'GET',
        });

        if (response.ok) {
        const taskInfo = await response.json();
        setCurrentTask({productName: taskInfo.product.name, productAmount: taskInfo.amount, instituition: taskInfo.instituition.name}); // Atualiza o estado com a tarefa gerada
      ...
      <View style={styles.mainContent}>
        ...
        {currentTask ? (
          ...
          <>
            ...
            <Button title="Generate Random Task" onPress={generateRandomTask} style={styles.generateButton} />
          </>
        )}
      </View>
    </View>
